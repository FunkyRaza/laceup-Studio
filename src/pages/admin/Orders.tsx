            ...
            columns={[
              {
                key: '_id',
                title: 'Order ID',
                render: (value) => <span className="font-mono text-xs font-medium text-gray-300">{value.slice(0, 8)}...</span>
              },
              {
                key: 'customerName',
                title: 'Customer',
                render: (_, record) => (
                  <div className="flex flex-col">
                    <span className="font-medium text-gray-300">{record.customerName}</span>
                    <span className="text-xs text-gray-400">{record.shippingAddress?.city || 'Unknown City'}</span>
                  </div>
                )
              },
              {
              ...
              },
              {
                key: 'total',
                title: 'Amount',
                render: (value) => <span className="font-bold text-gray-300">₹{value.toFixed(2)}</span>
              },
              {
                key: 'status',
                title: 'Status',
                render: (_, record) => (
                  <Select
                    value={record.status}
                    onValueChange={(val) => handleStatusChange(record._id, val)}
                  >
                    <SelectTrigger className={`h-8 w-fit border-none shadow-sm text-xs font-medium px-3 rounded-full ${record.status === 'delivered' ? 'bg-green-100 text-green-700 hover:bg-green-200' :
                      record.status === 'shipped' ? 'bg-blue-100 text-blue-700 hover:bg-blue-200' :
                        record.status === 'processing' ? 'bg-orange-100 text-orange-700 hover:bg-orange-200' :
                          record.status === 'cancelled' ? 'bg-red-100 text-red-700 hover:bg-red-200' :
              ...
              },
              {
                key: 'items',
                title: 'Items',
                render: (value) => <span className="text-gray-400 text-sm max-w-[200px] truncate">{value.map((i: any) => i.name).join(', ')}</span>
              },
              {
                key: 'actions',
                title: 'Actions',
                    ...
                    <Sheet>
                      <SheetTrigger asChild>
                        <Button
                          variant="ghost"
                          size="sm"
                          className="h-8 w-8 p-0 text-gray-400 hover:text-blue-400 hover:bg-blue-900/30 rounded-full"
                          onClick={() => setSelectedOrder(record)}
                        >
                          <Eye className="w-4 h-4" />
                        </Button>
                      </SheetTrigger>
                      <SheetContent className="overflow-y-auto bg-gray-800 text-gray-100 border-l border-gray-700 p-0 sm:max-w-md transition-all duration-300 ease-in-out !opacity-100">
                        {selectedOrder && (
                          <div className="h-full flex flex-col bg-gray-800 text-gray-100">
                            {/* Sheet Header */}
                            <div className="p-6 border-b border-gray-700">
                              <h2 className="text-xl font-bold text-gray-100 mb-1">Order Details</h2>
                            ...
                            <div className="flex-1 p-6 space-y-8 overflow-y-auto">
                              {/* Items */}
                              <div>
                                <h3 className="text-sm font-semibold text-gray-200 uppercase tracking-wider mb-4">Items Ordered</h3>
                                <div className="space-y-4">
                                  {selectedOrder.items.map((item: any, i: number) => (
                                    <div key={i} className="flex gap-4 items-start">
                                      <div className="w-16 h-16 rounded-lg bg-gray-700 border border-gray-600 overflow-hidden flex-shrink-0">
                                        <img src={getImageUrl(item.image)} alt="" className="w-full h-full object-cover" />
                                      </div>
                                      <div className="flex-1 min-w-0">
                                        <p className="font-medium text-gray-200 line-clamp-2">{item.name}</p>
                              ...
                              <div className="border-t border-gray-700 pt-6 space-y-3">
                                <div className="flex justify-between text-sm">
                                  ...
                                  <span className="font-medium text-gray-200">₹{selectedOrder.subtotal?.toFixed(2) || (selectedOrder.total * 0.9).toFixed(2)}</span>
                                </div>
                                <div className="flex justify-between text-sm">
                                  <span className="text-gray-400">Shipping</span>
                                  <span className="font-medium text-gray-200">₹{selectedOrder.shipping?.toFixed(2) || '0.00'}</span>
                                </div>
                                <div className="flex justify-between text-sm">
                                  <span className="text-gray-400">Tax</span>
                                  <span className="font-medium text-gray-200">₹{selectedOrder.tax?.toFixed(2) || (selectedOrder.total * 0.1).toFixed(2)}</span>
                                </div>
                    ...
                    <AlertDialog>
                      <AlertDialogTrigger asChild>
                        <Button
                          variant="ghost"
                          size="sm"
                          className="h-8 w-8 p-0 text-gray-400 hover:text-red-400 hover:bg-red-900/30 rounded-full"
                        >
